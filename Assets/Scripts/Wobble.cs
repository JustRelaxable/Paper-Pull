...
public class Wobble : MonoBehaviour
{
    ...
    public event Action onToiletPaperRipped;
    ...
    private void Update()
    {
        ...
        if(Mathf.Abs(wobbleAmountX) >= 0.4f && !isAI)
        {
            VibrationHelper.VibrateWithAmplitude(100, 120);
        }
#endif

        if(Mathf.Abs(wobbleAmountX) >= 0.9)
        {
            onToiletPaperRipped?.Invoke();
        }
